...
public class ConfigManager extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        for (int i = 0; i < classiCommand.size(); i++) {
            ...
            try {
                classCommand = (Command) Class.forName("it.tol.command." + voceMenu.getNomeClasse()).newInstance();
        ...
        surveyList = null;
        try {
            surveyList = db.getSurveys(Constants.GET_ALL_BY_CLAUSE, Constants.GET_ALL_BY_CLAUSE);
        }
        catch (WebStorageException wse) {
